...
class IsDark
  ...
  def self.magick_area_from_blob(x, y, blob, height, width, percent=80, range=(0..10)) # (x, y) is the left corner of an element over a blob, height and width is the element's size
    image = Magick::Image.read(blob).first
    dark = false
    dots = []
    range.each { |xx|
  ...
  def self.set_debug_data(show_info=false, draw_file=false)
	...
	def self.is_dark(x=nil, y=nil) #detects a dark color based on luminance W3 standarts ( https://www.w3.org/TR/WCAG20/#relativeluminancedef )
		dark = false
